...
#import "JHStringSize.h"

@implementation JHStringSize

/// 计算文字的高度，宽度固定
+ (CGFloat)height:(id)text font:(UIFont *)font fixedWidth:(CGFloat)width
{
    return calculateTextSize(CGSizeMake(width, 0), text, 0, font, NSTextAlignmentNatural, NSLineBreakByTruncatingTail, 0.0, CGSizeZero).height;
}

/// 计算文字的宽度，高度固定
+ (CGFloat)width:(id)text font:(UIFont *)font fixedHeight:(CGFloat)height
{
    return calculateTextSize(CGSizeZero, text, 1, font, NSTextAlignmentNatural, NSLineBreakByTruncatingTail, 0.0, CGSizeZero).width;
}
...
/// @return 返回自适应的最合适尺寸
static CGSize calculateTextSize(CGSize fitsSize, id _Nonnull text, NSInteger numberOfLines, UIFont * _Nullable font, NSTextAlignment textAlignment, NSLineBreakMode lineBreakMode, CGFloat minimumScaleFactor, CGSize shadowOffset) {
